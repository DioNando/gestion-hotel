...
class ReservationDay extends BaseController
{
		...
		if (isset($_POST['btn_validation'])) : {
				...
				if (!$this->validate($rules)) {
					...
					$newDataFacture = [
						...
						'date_facture_day' => date("Y-m-d H:i:s"),
					];
	...
	public function read()
	{
		...
		$data = [
			'reservations' => $reservations->select([
				...
				'(CASE 
					WHEN heure_arrive > NOW() AND debut_sejour = CURDATE() THEN "1"
					...
					END) AS etat',
				'(CASE 
				WHEN heure_arrive > NOW() AND debut_sejour = CURDATE() THEN TIMEDIFF(heure_arrive, CURTIME())
				WHEN heure_depart >= NOW() AND heure_arrive < NOW() AND debut_sejour = CURDATE() THEN TIMEDIFF(heure_depart, CURTIME())
				WHEN fin_sejour < CURDATE() OR heure_arrive < NOW() THEN "00:00:00"
				END) AS reste'
				
					])
					// WHEN debut_sejour != CURDATE() TIMEDIFF(CURTIME(), heure_depart)
			...
			'enAttente' => count($reservations->where('heure_arrive > NOW()')->where('debut_sejour = CURDATE()')->join('reservation_day', 'effectuer.ID_day = reservation_day.ID_day')->join('pour', 'pour.ID_day = reservation_day.ID_day')->join('planning', 'pour.ID_planning = planning.ID_planning')->findAll()),
	...
	public function infoSupplementaireDay($ID_day)
	{
		$data = [];
		$reservations = new effectuerModel();
		$data = $reservations->select(['*', 'DATE_FORMAT(date_reservation_day, "%d %b %Y à %H:%i") AS date_reservation_day', 'DATE_FORMAT(date_modif, "%d %b %Y à %H:%i") AS date_modif'])->where('effectuer.ID_day', $ID_day)->join('user', 'effectuer.ID_user = user.ID_user')->join('reservation_day', 'effectuer.ID_day = reservation_day.ID_day')->join('pour', 'pour.ID_day = reservation_day.ID_day')->join('planning', 'pour.ID_planning = planning.ID_planning')->first();
		return $data;
	}
	...
	public function update()
	{
		...
		if (isset($_POST['btn_modification'])) : {
				...
				if (!$this->validate($rules)) {
					...
					$id_planning = $pour->where('ID_day', $_POST['ID_day'])->join('planning', 'pour.ID_planning = planning.ID_planning')->join('concerner', 'concerner.ID_planning = planning.ID_planning')->first();
