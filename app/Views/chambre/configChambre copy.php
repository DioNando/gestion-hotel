    ...
    <table class="table table-hover table-striped" id="result">
        <thead>
            <tr>
                ...
                <th scope="col" class="text-end">Ancien avant</th>
                <?php if (session()->get('isUser') == 'Administrateur') : ?>
                    <th scope="col">Actions</th>
                <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="row">
                                <div class="col-1 ms-2 center">
                                    ...
                                    <!-- <?php if ($temp['statut_chambre'] == 'Libre') echo ('<i class="fas fa-tag text-success"></i>') ?> -->
                                    <!-- <?php if ($temp['statut_chambre'] == 'En attente') echo ('<i class="fas fa-exclamation-triangle text-danger"></i>') ?> -->
                                    <!-- <?php if ($temp['statut_chambre'] == 'Occupée') echo ('<i class="fas fa-house-user text-secondary"></i>') ?> -->
                                </div>
                                <div class="col text-start d-flex align-items-center"><?php echo ($temp['statut_chambre']) ?></div>
                            </div>
                        ...
                        <?php if (session()->get('isUser') == 'Administrateur') : ?>
                            <td>
                                <div class="center">
                                    <div>
                                        <button type="button" class="btn btn-outline-dark btn-icon btn-sm" data-bs-toggle="modal" data-bs-target="#modalChambreUpdate" onclick="updateChambre('<?php echo $temp['ID_chambre']; ?>' ,'update')"><i class="fas fa-pencil-alt"></i></button>
                                        <button type="button" class="btn btn-outline-danger btn-icon btn-sm" data-bs-toggle="modal" data-bs-target="#modalChambreDelete" onclick="deleteData('<?php echo $temp['ID_chambre']; ?>')"><i class="fas fa-trash-alt"></i></button>
                                        <!-- <button type="button" class="btn btn-outline-success btn-icon btn-sm" data-bs-toggle="modal" data-bs-target="#modalChambreInfo" onclick="infoData('<?php echo $temp['ID_chambre']; ?>')"><img src="assets/icons/info-circle-fill.svg" alt="Info"></button> -->
                                    </div>
                                </div>
                            </td>
                        <?php endif; ?>
    ...
    <div class="position-fixed bottom-0 end-0 p-4" style="z-index: 5">
        <div id="myToastSave" class="toast hide border" role="alert" aria-live="assertive" aria-atomic="true">
            ...
            <div class="toast-body alert alert-success alert-toast" role="alert">
                Les nouveaux tarifs ont été enregistré et seront utilisés aux prochaines réservations
            </div>
